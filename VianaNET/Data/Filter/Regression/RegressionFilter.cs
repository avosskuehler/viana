...
namespace VianaNET.Data.Filter.Regression
{
  ...
  public class RegressionFilter : FilterBase
  {
    ...
    /// </summary>
    [XmlIgnore]
    public Func<double, double> AusgleichsFunktion { get; set; }
    ...
    {
      return FitParameterMatrix[(int)RegressionType.Exponentiell, 0]
             * Math.Exp(x * FitParameterMatrix[(int)RegressionType.Exponentiell, 1])
             + FitParameterMatrix[(int)RegressionType.Exponentiell, 2];
    }
    ...
    {
      return FitParameterMatrix[(int)RegressionType.ExponentiellMitKonstante, 0]
             * Math.Exp(x * FitParameterMatrix[(int)RegressionType.ExponentiellMitKonstante, 1]);
    }
    ...
    {
      return FitParameterMatrix[(int)RegressionType.Logarithmisch, 0]
             * Math.Log(x * FitParameterMatrix[(int)RegressionType.Logarithmisch, 1]);
    }
    ...
    {
      return (FitParameterMatrix[(int)RegressionType.Quadratisch, 0] * x
              + FitParameterMatrix[(int)RegressionType.Quadratisch, 1]) * x
             + FitParameterMatrix[(int)RegressionType.Quadratisch, 2];
    }
    ...
    {
      return FitParameterMatrix[(int)RegressionType.Potenz, 0]
             * Math.Pow(x, FitParameterMatrix[(int)RegressionType.Potenz, 1]);
    }
    ...
    public static double AusgleichsReso(double x)
    {
      return FitParameterMatrix[(int)RegressionType.Resonanz, 0]
             / Math.Pow(
               1
               + FitParameterMatrix[(int)RegressionType.Resonanz, 1]
               * Math.Pow(x - FitParameterMatrix[(int)RegressionType.Resonanz, 2] / x, 2), 
               0.5);
    }
    ...
    {
      return FitParameterMatrix[(int)RegressionType.Sinus, 0]
             * Math.Sin(
               x * FitParameterMatrix[(int)RegressionType.Sinus, 1] + FitParameterMatrix[(int)RegressionType.Sinus, 2])
             + FitParameterMatrix[(int)RegressionType.Sinus, 3];
    }

    /// <summary>
    /// Calculates the value of the Regression function at argument x, when the function is a sinus-function and its
    ...
    {
      return FitParameterMatrix[(int)RegressionType.SinusGedämpft, 0]
             * Math.Sin(x * FitParameterMatrix[(int)RegressionType.SinusGedämpft, 1])
    ...
    public void GetBestRegressData(out RegressionType bestRegTyp, int negFlag)
    {
      ...
      for (aktRegTyp = RegressionType.Linear; aktRegTyp <= RegressionType.Resonanz; aktRegTyp++)
      {
        ...
        if (((aktRegTyp == RegressionType.Potenz) & (negFlag > 0))
            ...
            | ((aktRegTyp == RegressionType.Exponentiell) & (negFlag > 1)))
        {
          weiter = false;
        }
    ...
    /// </summary>
    public void UpdateLinefitFunctionData()
    {
    ...
    private void BestimmeQuadratFkt()
    {
      ...
      m[2, 2] = this.anzahl;
      v[2, 0] = sumY;
      Matrix lsg = Matrix.SolveLinear(m, v);
      double a = lsg[0, 0];
      double b = lsg[1, 0];
      double c = lsg[2, 0];
    ...
    private void BestimmeResonanzFkt()
    {
      ...
      int z = 9 - (int)Math.Floor((maxSchaetz - schaetzWert) / schaetzStep);
      ...
      while (iter < MaxIteration)
      {
        ...
        double b = param[1] / a;
        a = Math.Pow(a, -0.5);
        double c = schaetzWert;
        double fehler = 0;
        for (k = 0; k < this.anzahl; k++)
        {
          xi = this.WertX[k];
          hilfX = xi - c / xi;
          double hilf = a / Math.Pow(1 + b * hilfX * hilfX, 0.5) - this.WertY[k];
      ...
      out double schaetzStep)
    {
      ...
      schaetzStep = 10000;
      double hilf = maxSchaetz - schaetzWert;
    ...
    {
      double a = FitParameterMatrix[(int)regTyp, 0];
      ...
      {
        fktStr = fktStr.Replace('x', this.axisName);
        fktStr = fktStr.Replace('f', this.funcName);
        try
        {
