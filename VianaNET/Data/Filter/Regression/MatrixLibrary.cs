...
namespace VianaNET.Data.Filter.Regression
{
  ...
  /// <summary>
  ///   The vector dimension exception.
  /// </summary>
  ...
  public class Matrix
  {
      ...
      for (i = 0; i <= 50; i++)
      {
        ...
        for (ip = 0; ip <= n - 1; ip++)
        {
          for (iq = ip + 1; iq <= n; iq++)
          {
            ...
            if (i > 4 && (Math.Abs(d[ip, 0]) + g) == Math.Abs(d[ip, 0])
                && (Math.Abs(d[iq, 0]) + g) == Math.Abs(d[iq, 0]))
            {
              a[ip, iq] = 0.0;
            }
    ...
    public static bool IsEqual(double[,] Mat1, double[,] Mat2)
    {
      ...
      for (int i = 0; i <= Rows1; i++)
      {
        ...
        {
          if (Math.Abs(Mat1[i, j] - Mat2[i, j]) > eps)
          {
            return false;
          }
        }
    ...
    public static double[,] PINV(double[,] Mat)
    {
      ...
      for (i = 0; i <= svdS.GetUpperBound(0); i++)
      {
        if (i == 0)
        {
          Largest_Item = svdS[0, 0];
        }

        if (Largest_Item < svdS[i, i])
        {
          Largest_Item = svdS[i, i];
        }
      }

      if (m > n)
      {
        Tol = m * Largest_Item * EPS;
      }
      else
      {
        Tol = n * Largest_Item * EPS;
      }
    ...
    public static string PrintMat(double[,] Mat)
    {
      ...
      try
      {
        Find_R_C(Mat, out N_Rows, out N_Columns);
      }
      catch
      {
        throw new MatrixNullException();
      ...
      sol = string.Empty;
      LarString = string.Empty;
      OptiString = string.Empty;
    ...
    public static void SVD(double[,] Mat_, out double[,] S_, out double[,] U_, out double[,] V_)
    {
      ...
      try
      {
        Find_R_C(Mat_, out Rows, out Cols);
      }
      catch
      {
        throw new MatrixNullException();
    ...
    /// <returns> The division of the Matrix object and the scalar as a Matrix object </returns>
    ...
    /// <returns> The multiplication of the scalar and the Matrix object as a Matrix object </returns>
    public static Matrix operator *(Matrix Mat, double Value)
    {
      return new Matrix(ScalarMultiply(Value, Mat.in_Mat));
    }
